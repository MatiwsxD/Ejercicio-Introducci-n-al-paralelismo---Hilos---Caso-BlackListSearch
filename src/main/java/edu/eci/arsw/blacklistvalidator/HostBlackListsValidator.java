...
public class HostBlackListsValidator {
    ...
    public List<Integer> checkHost(String ipaddress, int n){
        ...
        int section = serverCount /numThreads;

        for (int i=0; i<n; i++) {
            threads[i] = new blacklistthread(i*section, Math.min((i+1)*section, serverCount-1), ipaddress);
            threads[i].start();
        }
        ...
        for (blacklistthread t: threads) {
            try {
                ...
                blackListOcurrences.addAll(t.getServers());
